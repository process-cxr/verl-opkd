...
class DataParallelOPKDActor(DataParallelPPOActor):
    ...
    def _forward_micro_batch_teacher(self, micro_batch, temperature, calculate_entropy=False):
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            merged_indices = torch.tensor(merged_indices, dtype=torch.int64, device=student_topk_index.device)  # [B, R, K]
        ...
        for _ in range(self.config.ppo_epochs):
            for mini_batch in mini_batches:
                ...
                if uids is not None:
                    ...
                    print(
                        f"[DBG][rank {rank}][cuda:{dev}] mini_batch uid summary: "
                        f"total={total}, "
                        f"unique={n_unique}, "
                    )
                    uid_cnt_preview = {k: cnt[k] for k in uniq[:5]}
                    print(
                        f"[DBG][rank {rank}][cuda:{dev}] mini_batch uid counts (preview): "
                        f"{uid_cnt_preview}"
                    )
                ...
                for micro_batch in micro_batches:
                    ...
                    responses = model_inputs["responses"]               # [B, R]
                    ...
                    student_sel_logits = torch.gather(
                        student_full_logits, dim=-1, index=merged_indices
                    )  # [B, R, K]
                    ...
                    loss_tok = loss_tok + loss_coverage

                    # ==========================================================
                    # >>> per-prompt power weighting across n rollouts <<<
                    lengths = response_mask.sum(dim=-1).clamp(min=1)  # [B]
                    score = (teacher_token_logp * response_mask).sum(dim=-1) / lengths  # [B], average logprob
                    ...
                    if self.use_power_weighting:
                        ...
                        w = torch.softmax(s, dim=0).detach()  # [B]
                    else:
                        # uniform weights
                        w = torch.ones(B, device=score.device, dtype=score.dtype)

                    # apply weights to token loss
                    w_tok = w.unsqueeze(-1)                                    # [B, 1]
                    weighted_mask = response_mask * w_tok                      # [B, R]

                    denom_weighted = weighted_mask.sum().clamp(min=1)
                    loss = (loss_tok * weighted_mask).sum() / denom_weighted
